...
function Dropdown({ value, onChange, options, placeholder = "Select...", className = "" }: DropdownProps) {
        ...
        <div className="bl-dropdown-menu">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
              className={`bl-dropdown-item ${option.value === value ? "selected" : ""} ${option.disabled ? "disabled" : ""}`}
