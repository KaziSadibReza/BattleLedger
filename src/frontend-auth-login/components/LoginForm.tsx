import React, { useState } from 'react';
import { Mail, Lock, Eye, EyeOff, ArrowRight } from 'lucide-react';
import OTPInput from './OTPInput';

interface FormSettings {
  colors?: Record<string, string>;
  typography?: Record<string, string>;
  labels?: {
    signInTitle?: string;
    signInSubtitle?: string;
    emailLabel?: string;
    passwordLabel?: string;
    signInButtonText?: string;
  };
  features?: {
    showLogo?: boolean;
    logoUrl?: string;
    showSocialLogin?: boolean;
    showOtpLogin?: boolean;
    showPasswordLogin?: boolean;
    defaultLoginMethod?: string;
  };
}

interface LoginFormProps {
  apiUrl: string;
  nonce: string;
  googleEnabled: boolean;
  otpEnabled: boolean;
  onSuccess: (user: any) => void;
  onError: (message: string) => void;
  onSwitchToSignup?: () => void;
  onGoogleLogin?: () => void;
  formSettings?: FormSettings;
}

type LoginMode = 'password' | 'otp';
type OTPStep = 'email' | 'verify';

export const LoginForm: React.FC<LoginFormProps> = ({
  apiUrl,
  nonce,
  googleEnabled,
  otpEnabled,
  onSuccess,
  onError,
  onSwitchToSignup,
  onGoogleLogin,
  formSettings,
}) => {
  // Get settings with defaults
  const labels = formSettings?.labels || {};
  const features = formSettings?.features || {};
  
  const signInTitle = labels.signInTitle || 'Welcome back';
  const signInSubtitle = labels.signInSubtitle || 'Sign in to your account';
  const emailLabel = labels.emailLabel || 'Email';
  const passwordLabel = labels.passwordLabel || 'Password';
  const signInButtonText = labels.signInButtonText || 'Sign In';
  
  const showOtpLogin = features.showOtpLogin !== false && otpEnabled;
  const showPasswordLogin = features.showPasswordLogin !== false;
  const showSocialLogin = features.showSocialLogin !== false && googleEnabled;
  const defaultMethod = features.defaultLoginMethod || 'otp';
  const showLogo = features.showLogo && features.logoUrl;
  
  const [mode, setMode] = useState<LoginMode>(showOtpLogin && defaultMethod === 'otp' ? 'otp' : 'password');
  const [otpStep, setOtpStep] = useState<OTPStep>('email');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [remember, setRemember] = useState(false);
  const [loading, setLoading] = useState(false);
  const [, setOtpExpiresIn] = useState(0);

  const handlePasswordLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!email || !password) {
      onError('Please enter your email and password');
      return;
    }

    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/login`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-WP-Nonce': nonce,
        },
        credentials: 'include',
        body: JSON.stringify({ email, password, remember }),
      });

      const data = await response.json();

      if (data.success) {
        onSuccess(data.user);
      } else {
        onError(data.message || 'Login failed');
      }
    } catch (error) {
      onError('Network error. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleSendOTP = async (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!email) {
      onError('Please enter your email');
      return;
    }

    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/otp/send`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-WP-Nonce': nonce,
        },
        credentials: 'include',
        body: JSON.stringify({ email, type: 'login' }),
      });

      const data = await response.json();

      if (data.success) {
        setOtpStep('verify');
        setOtpExpiresIn(data.expires_in || 600);
      } else {
        onError(data.message || 'Failed to send code');
      }
    } catch (error) {
      onError('Network error. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleVerifyOTP = async (otp: string) => {
    if (otp.length < 6) return;

    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/otp/login`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-WP-Nonce': nonce,
        },
        credentials: 'include',
        body: JSON.stringify({ email, otp }),
      });

      const data = await response.json();

      if (data.success) {
        onSuccess(data.user);
      } else {
        onError(data.message || 'Invalid code');
      }
    } catch (error) {
      onError('Network error. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleGoogleLogin = async () => {
    if (!googleEnabled) return;

    try {
      const response = await fetch(`${apiUrl}/google/url`, {
        credentials: 'include',
      });
      const data = await response.json();

      if (data.success && data.url) {
        window.location.href = data.url;
      } else {
        onError('Google login is not available');
      }
    } catch (error) {
      onError('Failed to initiate Google login');
    }
  };

  // OTP Verification step
  if (mode === 'otp' && otpStep === 'verify') {
    return (
      <div className="ak-auth-form">
        <div className="ak-form-header">
          <h2>Enter verification code</h2>
          <p>We sent a 6-digit code to <strong>{email}</strong></p>
        </div>

        <OTPInput
          length={6}
          onComplete={handleVerifyOTP}
          disabled={loading}
          autoFocus
        />
        
        {loading && (
          <div className="ak-submit-btn" style={{ pointerEvents: 'none' }}>
            <span className="ak-spinner" />
            <span>Verifying...</span>
          </div>
        )}

        <div className="ak-resend-otp">
          <span>Didn't receive the code?</span>
          <button
            type="button"
            onClick={handleSendOTP}
            disabled={loading}
          >
            Resend
          </button>
        </div>

        <div className="ak-form-footer">
          <p>
            <button type="button" onClick={() => setOtpStep('email')}>
              Use a different email
            </button>
          </p>
        </div>
      </div>
    );
  }

  return (
    <div className="ak-auth-form">
      {/* Logo */}
      {showLogo && (
        <div className="ak-form-logo">
          <img src={features.logoUrl} alt="Logo" />
        </div>
      )}
      
      <div className="ak-form-header">
        <h2>{signInTitle}</h2>
        <p>{signInSubtitle}</p>
      </div>

      {/* Google Login Button */}
      {showSocialLogin && (
        <>
          <button
            type="button"
            className="ak-social-btn google"
            onClick={onGoogleLogin || handleGoogleLogin}
          >
            <svg viewBox="0 0 24 24" width="20" height="20">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google
          </button>

          <div className="ak-divider">
            <span>or</span>
          </div>
        </>
      )}

      {/* Mode Toggle - show only if both methods are enabled */}
      {showOtpLogin && showPasswordLogin && (
        <div className="ak-method-toggle">
          <button
            type="button"
            className={mode === 'otp' ? 'active' : ''}
            onClick={() => setMode('otp')}
          >
            <Mail size={16} />
            Email Code
          </button>
          <button
            type="button"
            className={mode === 'password' ? 'active' : ''}
            onClick={() => setMode('password')}
          >
            <Lock size={16} />
            Password
          </button>
        </div>
      )}

      {mode === 'password' ? (
        <form onSubmit={handlePasswordLogin}>
          <div className="ak-form-group">
            <label className="ak-form-label" htmlFor="login-email">{emailLabel}</label>
            <div className="ak-input-wrapper">
              <span className="ak-input-icon"><Mail size={18} /></span>
              <input
                type="email"
                id="login-email"
                className="ak-form-input"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="you@example.com"
                autoComplete="email"
                required
              />
            </div>
          </div>

          <div className="ak-form-group">
            <label className="ak-form-label" htmlFor="login-password">{passwordLabel}</label>
            <div className="ak-input-wrapper ak-password-wrapper">
              <span className="ak-input-icon"><Lock size={18} /></span>
              <input
                type={showPassword ? 'text' : 'password'}
                id="login-password"
                className="ak-form-input"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                placeholder="••••••••"
                autoComplete="current-password"
                required
              />
              <button
                type="button"
                className="ak-password-toggle"
                onClick={() => setShowPassword(!showPassword)}
              >
                {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
              </button>
            </div>
          </div>

          <div className="ak-terms-check">
            <input
              type="checkbox"
              id="remember-me"
              checked={remember}
              onChange={(e) => setRemember(e.target.checked)}
            />
            <label htmlFor="remember-me">Remember me</label>
          </div>

          <button
            type="submit"
            className="ak-submit-btn"
            disabled={loading}
          >
            {loading ? (
              <span className="ak-spinner" />
            ) : (
              <>
                {signInButtonText}
                <ArrowRight size={18} />
              </>
            )}
          </button>

          <div className="ak-forgot-password">
            <a href="#">Forgot password?</a>
          </div>
        </form>
      ) : (
        <form onSubmit={handleSendOTP}>
          <div className="ak-form-group">
            <label className="ak-form-label" htmlFor="otp-email">{emailLabel}</label>
            <div className="ak-input-wrapper">
              <span className="ak-input-icon"><Mail size={18} /></span>
              <input
                type="email"
                id="otp-email"
                className="ak-form-input"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="you@example.com"
                autoComplete="email"
                required
              />
            </div>
          </div>

          <button
            type="submit"
            className="ak-submit-btn"
            disabled={loading}
          >
            {loading ? (
              <span className="ak-spinner" />
            ) : (
              <>
                Send Code
                <ArrowRight size={18} />
              </>
            )}
          </button>
        </form>
      )}

      {onSwitchToSignup && (
        <div className="ak-form-footer">
          <p>
            Don't have an account?{' '}
            <button type="button" onClick={onSwitchToSignup}>
              Sign Up
            </button>
          </p>
        </div>
      )}
    </div>
  );
};

export default LoginForm;
